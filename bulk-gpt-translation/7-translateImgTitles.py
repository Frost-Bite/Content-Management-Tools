            ...
            if not original_alt.strip():
                ...
                while not success and retries > 0:
                    try:
                        ...
                        print('Translated text:', translated_alt)
                        img['title'] = translated_alt
                        success = True
...
# Read the xlsx file
df = pd.read_excel('6-imgaltstrans.xlsx', engine='openpyxl')

# Replace alts and translate them
for index, row in df.iterrows():
    if not row['NewContentnenalts'] or not isinstance(row['NewContentnenalts'], str):
        print(f'Skipping empty cell at index {index}')
        continue
    df.at[index, 'NewContentnentitles'] = extract_and_translate_alts(row['NewContentnenalts'])
    # Write to a new xlsx file
    df.to_excel('7-imgtitlestrans.xlsx', index=False, engine='xlsxwriter')
